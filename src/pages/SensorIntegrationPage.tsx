...
export default function SensorIntegrationPage() {
  ...
  const saveSensorConfig = async () => {
    ...
    if (error) {
      ...
      toast({ title: 'Sensor configuration saved' });
    }
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <Select value={sensorType} onValueChange={setSensorType}>
                ...
                <SelectContent>
                  {sensorTypes.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={connectionType} onValueChange={setConnectionType}>
                ...
                <SelectContent>
                  {connectionTypes.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <Input value={deviceAddress} onChange={e => setDeviceAddress(e.target.value)}
                placeholder="e.g. 192.168.1.100:8080" className="bg-background/50 border-border/50" />
            </div>
            <div>
              <label className="text-sm text-muted-foreground mb-1 block">Unit (auto-detected if empty)</label>
              <Input value={unit} onChange={e => setUnit(e.target.value)}
                placeholder="e.g. °C, kPa, RPM" className="bg-background/50 border-border/50" />
            </div>
          </div>
      ...
      {sensorType && (
        <Card className="glass border-glow animate-slide-up">
          <CardHeader className="pb-0">
            <button onClick={() => setShowSensorGuide(!showSensorGuide)}
              className="flex items-center justify-between w-full text-left">
              <CardTitle className="text-sm uppercase tracking-wider text-muted-foreground">
                {sensorType} Sensor Guidelines
              </CardTitle>
